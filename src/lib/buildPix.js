export default function buildPixPayload(obj) {
  ...
  const additionalDataFieldTemplate = formatField('62',
    formatField('05', message.slice(0, 25))
  );

  // Monta o payload sem CRC
  const payload = payloadFormatIndicator + pointOfInitiationMethod + merchantAccountInfo +
    merchantCategoryCode + transactionCurrency + transactionAmount +
    countryCode + merchantName + merchantCity + additionalDataFieldTemplate;
